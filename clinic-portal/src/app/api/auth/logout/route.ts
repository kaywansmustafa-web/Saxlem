...
export async function POST(request: Request): Promise<Response> {
  ...
  let configuration: ReturnType<typeof authenticationComposition>["configuration"];
  try {
    ...
    const value = request.headers
      ...
      ?.slice(name.length + 1);
    try {
      await composition.service.logout(value);
    } catch {
      // Local logout remains authoritative for the browser when the backend is down.
    }
