...
const roles: readonly PortalStaffRole[] = [
  "receptionist",
  "doctor",
  "clinicManager",
...
describe("Sprint 13K-C protected route matrix", () => {
  ...
  it("has a manifest entry for every policy and unique direct URL", () => {
    expect(new Set(routePolicies.map((item) => item.id)).size).toBe(
      routePolicies.length,
    );
    expect(new Set(routePolicies.map((item) => item.segment)).size).toBe(
      routePolicies.length,
    );
  });
  ...
  it.each([
    ...
    expect(safeReturnPath(candidate, "/ku/login")).toBe("/ku/login"),
  );
