...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    JOIN public.clinics c ON c.id = dca.clinic_id
      AND c.organization_id = dca.organization_id
    JOIN public.organizations o ON o.id = dca.organization_id
    WHERE dca.organization_id = p_organization_id
      AND dca.clinic_id = p_clinic_id
      AND dca.doctor_id = p_doctor_id
  ...
  INSERT INTO public.queue_sessions (
    organization_id, clinic_id, doctor_id, operational_date,
    effective_timezone, status
  ) VALUES (
    p_organization_id, p_clinic_id, p_doctor_id, p_operational_date,
    p_effective_timezone, 'notStarted'
...
BEGIN
  IF NOT EXISTS (
    ...
    JOIN public.appointments a
      ...
      AND a.clinic_id = qs.clinic_id
      AND a.doctor_id = qs.doctor_id
    ...
    WHERE qs.id = p_queue_session_id
      AND qs.organization_id = p_organization_id
      AND qs.clinic_id = p_clinic_id
      AND qs.status IN ('open','paused')
      AND qs.next_ticket = p_ticket_number
      AND a.id = p_appointment_id
      AND a.patient_profile_id = p_patient_profile_id
  ...
  INSERT INTO public.queue_entries (
    organization_id, clinic_id, queue_session_id, appointment_id,
    arrival_id, patient_profile_id, ticket_number, status
  ) VALUES (
    p_organization_id, p_clinic_id, p_queue_session_id, p_appointment_id,
    p_arrival_id, p_patient_profile_id, p_ticket_number, 'waiting'
  ) RETURNING id INTO created_id;
  RETURN created_id;
END
$function$;

REVOKE ALL ON FUNCTION queue_create_session(uuid,uuid,uuid,date,text) FROM PUBLIC;
REVOKE ALL ON FUNCTION queue_create_entry(uuid,uuid,uuid,uuid,uuid,uuid,integer) FROM PUBLIC;
