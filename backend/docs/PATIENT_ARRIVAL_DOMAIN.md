...
Configuration accepts only integers from 0 through 1440. The policy is timezone-safe because persisted appointment instants and comparisons are UTC; clinic-local schedule interpretation remains owned by the Appointment/Schedule domains.
...
Commands acquire an idempotency advisory lock and doctor/patient advisory locks, then lock the appointment row. Optimistic version checks and the database lifecycle trigger prevent duplicate or concurrent arrival transitions. An identical completed request replays its stored response. Reusing a key with a different request hash returns deterministic conflict; partially executing commands roll back atomically.
